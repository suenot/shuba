...
test('resolveTarget maps a known provider to endpoint + strips it from model', () => {
  const a8e = resolveTarget('a8e/a8e-1.0-pro');
  assert.equal(a8e.model, 'a8e-1.0-pro');
...
test('applyRoute resolves a route target to model + upstream', () => {
  ...
  assert.equal(body.model, 'a8e-1.0-pro');
  assert.equal(stats.routedModel, 'a8e-1.0-pro');
