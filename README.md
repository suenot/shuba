...
`orchestrator/` is mirrored to <https://github.com/suenot/shuba> via git
subtree split (run from the monorepo root):

```bash
git subtree split --prefix=orchestrator -b shuba-mirror
git push git@github.com:suenot/shuba.git shuba-mirror:main --force
git branch -D shuba-mirror
```

The monorepo is the source of truth; the mirror's history is regenerated by
the split (hence the `--force`). The external proxies shuba wraps are installed separately —
